  ...
  <div id="login_zone">
    ...
    <div class="login_zone_container">
      <h1 ref="h1" id="h1" style="text-align:center; font-size: xx-large; color: cornflowerblue">
        <img v-bind:src="require(`@/assets/${icon_pi}`)" style="width: 25px; height: 25px" class="icons" alt="PI">{{login_zone_text}}</h1>
    ...
    <!--    Lista conturi-->
    <div v-for="(card,index) in resurse_backend_pinterest" :key="index" class="card" id="card" ref="card">
...
export default {
  ...
  methods: {
    login() {
      if (((!/^[^@]+@\w+(\.\w+)+\w$/.test(this.email_input)) === false) && (this.password_input !== '')) {
        this.status = 'Active'
        this.resurse_backend_pinterest.push({
        ...
        localStorage.setItem('Resurse Backend Pinterest', JSON.stringify(this.resurse_backend_pinterest))
        this.email_input = ''
        this.password_input = ''
      }
    },
    logout(index) {
      this.resurse_backend_pinterest[index].status = "Inactive"
    ...
    relogin(index) {
      this.resurse_backend_pinterest[index].status = 'Active'
    ...
    remove(button, index) {
      ...
      if (button === 'remove') {
        ...
        this.resurse_backend_pinterest.splice(index, 1)
        localStorage.setItem('Resurse Backend Pinterest', JSON.stringify(this.resurse_backend_pinterest))
      }
      if (button === 'cancel') {
        this.$refs.modal[index].style.display = "none"
  ...
  mounted() {
    let resurse_backend_local = localStorage.getItem("Resurse Backend Pinterest")
    this.resurse_backend_pinterest = JSON.parse(resurse_backend_local)

    if (resurse_backend_local === null) {
      resurse_backend_local = []
      this.resurse_backend_pinterest = resurse_backend_local
    } else {
      resurse_backend_local = localStorage.getItem("Resurse Backend Pinterest")
    }
  }
}
